ConfigMgr Client Health Version: 0.8.2
...
Note: Script version 0.8.2 requires database version 0.7.5. Execute query in CreateDatabase.sql to create or update the database if database is at a lower level.
SQL Query to get database version: SELECT Name, Version FROM Configuration

Changelog:
0.8.2 - Fixed a bug where logging directly to SQL database would not work.
